...
def generate_launch_description():
    ...
    control_launch_path = PathJoinSubstitution(
        [FindPackageShare('mini_pupper_control'), 'launch', 'vel_to_servo_controller.launch.py']
    )
    ...
    ld.add_action(declare_hardware_connected)

    ld.add_action(rviz2_node)
    ld.add_action(control_launch)
    ld.add_action(servo_interface_launch)
